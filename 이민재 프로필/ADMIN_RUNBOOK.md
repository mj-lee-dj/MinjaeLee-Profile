...
관리자 화면과 `/api/admin` 서버 진입점이 운영 배포에서 활성화되어 있다. 관리자 페이지는 정상 제공되고, 직접 교차 출처 API 호출은 403으로 차단된다. 저장은 배포된 관리자 페이지의 로그인·CSRF 보호 흐름에서만 실행한다. 환경변수 값은 Git이나 문서에 기록하지 않는다.
